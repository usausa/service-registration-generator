...
public sealed class Generator : IIncrementalGenerator
{
    ...
    private static IEnumerable<AttributeModel> EnumerateAttribute(IMethodSymbol methodSymbol)
    {
        ...
        {
            var pattern = attributeData.ConstructorArguments[0].Value?.ToString() ?? string.Empty;
            ...
            foreach (var parameter in attributeData.NamedArguments)
            {
                ...
                switch (name)
                {
                    case nameof(ServiceRegistrationAttribute.Assembly):
                        assembly = value.ToString();
                        break;
                    case nameof(ServiceRegistrationAttribute.Namespace):
                        ns = value.ToString();
                        break;
                }
            }

            yield return new AttributeModel(pattern, assembly, ns);
        }
    }
